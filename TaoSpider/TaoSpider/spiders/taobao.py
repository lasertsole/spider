    ...
    def parse(self, response):
        ...
        try:
            if tab.ele("x://div[@class='nc_wrapper']"):
                for _ in range(3):
                    wrapper_width = tab.run_js("document.querySelector('.nc_wrapper').offsetWidth", as_expr=True)#as_expr=True结果直接返回不用return
                    ...
                    tab.actions.move(offset_x=need_move, offset_y=round(random.uniform(1.0, 3.0), 0), duration=.2)
